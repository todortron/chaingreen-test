...
from chaingreen.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_chaingreen_ca_crt_key,
    make_ca_cert,
...
from chaingreen.util.ssl import (
...
from chaingreen.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    all_targets = []
    stop_searching_for_farmer = "cgn_target_address" not in config["farmer"]
    stop_searching_for_pool = "cgn_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("cgn_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("cgn_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "cgn_target_address" not in config["farmer"]:
        print(f"Setting the xch destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["cgn_target_address"] = all_targets[0]
    elif config["farmer"]["cgn_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['cgn_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "cgn_target_address" not in config["pool"]:
        print(f"Setting the xch destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["cgn_target_address"] = all_targets[0]
    elif config["pool"]["cgn_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['cgn_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root_path: Path):
    ...
    chaingreen_ca_crt, chaingreen_ca_key = get_chaingreen_ca_crt_key()
    chaingreen_ca_crt_path = ca_dir / "chaingreen_ca.crt"
    chaingreen_ca_key_path = ca_dir / "chaingreen_ca.key"
    write_ssl_cert_and_key(chaingreen_ca_crt_path, chaingreen_ca_crt, chaingreen_ca_key_path, chaingreen_ca_key)
    ...
    chaingreen_ca_crt, chaingreen_ca_key = get_chaingreen_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, chaingreen_ca_crt, chaingreen_ca_key, False, overwrite=False)
